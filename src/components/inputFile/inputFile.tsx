...
export const InputFile = ({label,value,touched,errors, onChange, ...props}: IProps) =>{


    return (
        <div>
        <span className="text-custom-text">{label}</span>
        <div className="mt-2 flex justify-center rounded-lg border border-solid border-custom-lilic px-2 py-2 bg-white h-[96px] items-center">
        <div className="text-center ">
          <div className="flex text-sm/6 gap-2">
          {value?.name ? 
          <>
            <span className="text-custom-text text-base font-normal">{value.name}</span>
            <button className="inline-flex items-center" onClick={onChange}>
                <svg className="fill-[#000853] hover:fill-custom-red" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          :<>
            <label  className="relative cursor-pointer rounded-md bg-white font-semibold text-indigo-600 focus-within:outline-hidden">
                <span className="text-custom-accent text-base font-normal underline ">Upload a file</span>
                <input 
                    required
                    {...props}
                    onChange={onChange}
                ...
                <p className="text-custom-grey-purple text-base font-normal ">or drag and drop</p>
            </>
            }
          </div>
        </div>
