...
export const Input = ({label,value,touched,errors, onChange, ...props}: IProps) =>{

    return (
        <div className="flex flex-col">
            {(label && 
                <span className="text-custom-text text-base">{label}</span>
            )}
            <input
                required
                {...props}
                onChange={onChange}
                className={`${touched && errors ? 'ring-1 ring-custom-red border-custom-red bg-custom-red-bg' : ' border-gray-300'}  px-3 py-2 border rounded-lg text-gray-900 focus:outline-none focus:ring-1 focus:ring-custom-accent focus:border-custom-accent`}
                >
            </input>
            {touched && errors && <div  className="text-sm text-custom-text font-normal flex mt-1 gap-1">
